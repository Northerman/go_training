package task //note that only 1 .go file be in a package (folder) **can have multiple files but has to be the same package!!
...
func init() { //danger when run test! because every test case will run on the database!  (but if put init in main it's ok! becuase it only run when program run i think)
	var err error
...
func GetTodosHandler(c *gin.Context) { // Public!!
